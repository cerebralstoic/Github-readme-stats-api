...
<g>
  ...
  <text x="${x}" y="${y + 52}" text-anchor="middle"
    style="font-family:'Monaco','Menlo',monospace;
           font-size:15px;font-weight:700;fill:${theme.title}">
    ${value}
  </text>
</g>`;
}

function renderStatsTimeline(stats, theme) {
  const username = sanitizeValue(stats.username, "User");
  const repoCount = Number(stats.repoCount) || 0;
  ...
  const pullRequests = Number(stats.pullRequests) || 0;
  const accountAge = sanitizeValue(stats.accountAge, "N/A");
  const languages = getSafeArray(stats.topLanguages, []).slice(0, 3);
  const primaryLanguage = sanitizeValue(languages[0]?.name, "N/A");

  const milestones = [
    { label: "Repos", value: repoCount, target: 50 },
    { label: "Stars", value: stars, target: 10 },
    { label: "Commits", value: commits, target: 250 },
    { label: "Contribs", value: contributions, target: 500 },
    { label: "Followers", value: followers, target: 25 }
  ];

  const completion = milestones.reduce((sum, item) => {
    return sum + Math.min(1, item.value / item.target);
  }, 0) / milestones.length;

  const progressWidth = 588;
  const filledWidth = Math.max(26, progressWidth * completion);
  const startX = 86;
  const spacing = progressWidth / (milestones.length - 1);

  return `
<svg width="820" height="320" viewBox="0 0 820 320" xmlns="http://www.w3.org/2000/svg">
  <defs>
    <linearGradient id="timeline-bg" x1="0" y1="0" x2="1" y2="1">
      <stop offset="0%" stop-color="${theme.background}"/>
      <stop offset="100%" stop-color="${theme.border}"/>
    </linearGradient>
    <linearGradient id="timeline-track" x1="0" y1="0" x2="1" y2="0">
      <stop offset="0%" stop-color="${theme.accent}" stop-opacity="1"/>
      <stop offset="100%" stop-color="${theme.title}" stop-opacity="0.45"/>
    </linearGradient>
  </defs>

  <rect width="100%" height="100%" rx="22" fill="url(#timeline-bg)"/>
  <rect x="14" y="14" width="792" height="292" rx="18"
  ...
  <text x="34" y="104"
    ...
    ${accountAge} yrs on GitHub / primary stack: ${primaryLanguage}
  </text>

  <rect x="34" y="128" width="752" height="64" rx="18"
        fill="${theme.border}" opacity="0.14"/>
  <rect x="34" y="128" width="752" height="64" rx="18"
        fill="${theme.background}" opacity="0.18"
        stroke="${theme.border}" stroke-opacity="0.38"/>

  <text x="56" y="154"
    style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;
           font-size:11px;letter-spacing:0.8px;text-transform:uppercase;fill:${theme.muted}">
    Current Snapshot
  </text>
  <text x="56" y="178"
    style="font-family:'Monaco','Menlo',monospace;
           font-size:20px;font-weight:700;fill:${theme.title}">
    ${commits}
  </text>
  <text x="132" y="178"
    style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;
           font-size:14px;fill:${theme.text}">
    commits / ${contributions} contributions / ${pullRequests} PRs
  </text>
  ...
  <rect x="${startX}" y="222" width="${progressWidth}" height="6" rx="999"
        fill="${theme.border}" opacity="0.35"/>
  <rect x="${startX}" y="222" width="${filledWidth}" height="6" rx="999"
        fill="url(#timeline-track)"/>

  ${milestones.map((item, index) => {
    const x = startX + spacing * index;
    const active = x <= startX + filledWidth + 2;
    return timelineNode(x, 225, sanitizeValue(item.label), sanitizeValue(item.value, "0"), active, theme);
  }).join("")}

  <text x="34" y="292"
    style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;
           font-size:12px;fill:${theme.muted}">
    milestones across profile growth, activity, and community reach
  </text>
</svg>`;
}
...
export function renderStatsSVG(stats, style = "gradient", theme) {
  switch (style.toLowerCase()) {
    ...
    case "timeline":
      return renderStatsTimeline(stats, theme);
    case "badge":
